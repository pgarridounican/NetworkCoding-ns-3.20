...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  if (ipHeader.GetTtl () == 0)
	...
	// Pablo/David/Ramón --> IP Forwarding: We need to intercept the Network Coding packets, in order not to replicate the information delivery.
    //Discard all the Network Coding packets (The NetworkCodingL4Protocol overhearing scheme will take care of them)
    // 100 -> IntraFlowNetworkCodingProtocol (UDP only)

    if (ipHeader.GetProtocol() == 100)
    {

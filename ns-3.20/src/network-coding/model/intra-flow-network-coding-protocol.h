...
struct IntraFlowNetworkCodingStatistics
{
	...
	std::vector<float> rankTime;
	std::vector<float> inverseTime;
...
public:
	...
	/**
	 * arg1: Packet
	 * arg2: 0 ENCODING, 1 SENDNOCODED, 2 RECEIVE, 3 RECEIVEACK, 4 DECODE, 5 SENDACK, 6 RECEIVENCODED,7 SENDRECODE, 8 BUFFERTXDATA, 9 RECEIVEOUTOFFRAGMENT,
	 * arg3: Node ID
	 ...
	 * in the stack.
	 */
	virtual enum IpL4Protocol::RxStatus Receive (Ptr<Packet> packet, Ipv4Header const &header, Ptr<Ipv4Interface> incomingInterface);  	//Old version, inherited form base class IpL4Protocol
	 ...
	 * in the stack.
	 *
	 * Not working with ip6
	 *
	 */
	virtual enum IpL4Protocol::RxStatus Receive (Ptr<Packet> packet, Ipv6Header const &header, Ptr<Ipv6Interface> incomingInterface);  	//Old version, inherited form base class IpL4Protocol
	 ...
	 */
	void Recode (u_int16_t flowId, bool last);
...
//	inline IntraFlowNetworkCodingStatistics GetStats () {return m_stats;}
	...
	/*
	 * Insert a row in a finite field Matrix.
	 * A -> Matrix
	 * vector -> Vector must be inserted in the matrix
	 * rowNumber -> Number of the row where the vector is inserted
	 */
	void InsertRowGFMatrix(mzed_t *A, vector<u_int8_t> *vector, int rowNumber);
...
protected:
	 ...
	 * \returns True if that is the case; false otherwise
	 */
	bool AmISource(const Ipv4Address &source);
	/**
...
private:
	...
	bool m_systematic;								//True = Systematic
	bool m_overhearing;
	Time m_bufferTimeout;							//Time during which the protocol will wait until the buffer has at least K packets
	uint16_t m_startRecoding;						//When the intermediate node start recode packets
...
//	IntraFlowNetworkCodingStatistics m_stats;
	...
	//Values to store the links state in order to calculate the Tx probability
	map <int, vector<float> > m_channelfloatFer;

	//Generador de números aleatorios
	Ptr<UniformRandomVariable> m_randomGenerator;
};

...
void IntraFlowNetworkCodingHeader::Serialize (Buffer::Iterator start) const
{
	...
	if (m_k)
	{
		...
		for (u_int32_t j=0; j<m_k; j++)
		{
			std::bitset<8160> temp (m_vector[j]);
			temp <<= j*m_q;
			binaryVector |= temp;
		}
		...
		for (u_int32_t j=0; j < ceil((float) (m_k*m_q)/8); j++)
		{
...
u_int32_t IntraFlowNetworkCodingHeader::Deserialize (Buffer::Iterator start)
{
	...
	if (m_k)
	{
		...
		for (u_int32_t j=0; j<headerVector.size(); j++)
		{
			std::bitset<8160> temp (headerVector[j]);
			temp <<= j*8;
			deserializedVector |= temp;
		}
		for (u_int8_t j=0; j< m_k; j++)
		{
...
{
	os << " Tx = " << (int) m_tx << " K = " << (int) m_k << " q = " << (int)m_q << " Source Port " << m_sourcePort << " Destination Port " << m_destinationPort << " Fragmento= " << (int) m_nfrag;
	os << " Vector: ";

...
private:
	u_int8_t m_k;              // Fragment size --> Number of packets encoded altogether
	...
	u_int16_t m_nfrag;          // Fragment number
	u_int8_t m_tx;              // Type of transmission FLAGS: First byte Data Transmission (0)/ ACK (1); Second byte Long(1)/Short(1) Header

	u_int16_t m_sourcePort;
	u_int16_t m_destinationPort;

	std::map<uint8_t,float> m_vectorForwarders;
...
class IntraFlowNetworkCodingTag : public Tag
{
public:
  static TypeId GetTypeId (void);
  virtual TypeId GetInstanceTypeId (void) const;
  ...
  void SetIdNode (uint8_t value);
  uint8_t GetIdNode (void) const;
